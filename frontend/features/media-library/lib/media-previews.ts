...
export function shouldPollMediaListPreviews(
  media: MediaListItemResponseData
) {
  return media.status === "UPLOADED" && media.type === "VIDEO" && !media.thumbnail
}
...
export function shouldPollMediaDetailPreviews(media: MediaDetailResponseData) {
  if (media.status !== "UPLOADED" || !requiresGeneratedDetailPreviews(media.type)) {
    return false
  }
...
export function parseMediaPreviewSpriteSheets(
  ...
  return media.previews.thumbnailSprites.flatMap((asset) => {
    ...
    const effectiveIntervalSeconds = getNumber(metadata.effectiveIntervalSeconds)
...
export function resampleWaveformPeaks(peaks: number[], barCount: number) {
  ...
  return Array.from({ length: barCount }, (_, barIndex) => {
    ...
    for (let index = startIndex; index < endIndex && index < peaks.length; index += 1) {
