...
export function getMediaUploadDescriptor(file: File): {
  ...
  if (
    [".mp3", ".mp4", ".m4a", ".wav", ".webm", ".ogg", ".flac"].includes(
      extension,
    ) &&
  ...
  get(
    ...
    return unwrapApiResponse(
      authenticatedApiClient.get<
        ApiSuccess<{ media: MediaDetailResponseData }>
  ...
  const worker = async () => {
    while (nextPartIndex < session.parts.length) {
      ...
      if (typeof etag !== "string" || !etag.trim()) {
        throw new Error(
          `Storage did not expose an ETag for part ${part.partNumber}`,
        );
      }
...
export async function uploadMediaFile({
  ...
  try {
    ...
    const parts = await uploadMultipart(file, session, signal, onProgress);
    const { media } = await mediaService.completeUpload(
      workspaceId,
      session.mediaId,
      {
