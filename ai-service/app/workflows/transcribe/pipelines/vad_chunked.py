...
from app.workflows.transcribe.errors import InvalidTranscribeResultError
from app.workflows.transcribe.planning import OfflineAsrWindow, plan_vad_asr_windows
...
def run_vad_chunked_pipeline(
    *,
    ...
    if not audio.samples:
        raise InvalidTranscribeResultError("audio must include decoded samples")
    ...
    if request.options.enable_vad:
        ...
        if not speech_regions_pass_gate(
            ...
            raise InvalidTranscribeResultError("audio does not contain enough speech")
