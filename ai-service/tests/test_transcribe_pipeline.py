...
from app.workflows.transcribe.pipeline import (
    ...
    select_transcribe_pipeline,
)
from app.workflows.transcribe.planning import OfflineAsrWindow
...
def test_selects_diarized_turns_pipeline() -> None:
    assert (
        select_transcribe_pipeline(ASR_STRATEGY_DIARIZED_TURNS)
...
def test_selects_vad_chunked_pipeline() -> None:
    assert (
        select_transcribe_pipeline(ASR_STRATEGY_VAD_CHUNKED) is run_vad_chunked_pipeline
    )


def test_rejects_unknown_asr_strategy() -> None:
    with pytest.raises(UnsupportedAsrStrategyError):
        select_transcribe_pipeline("unknown")
