...
def test_faster_whisper_adapter_maps_segments(
    ...
    assert model_calls[1]["kwargs"]["word_timestamps"] is False
...
def test_faster_whisper_adapter_maps_optional_word_timestamps(
    monkeypatch,
    tmp_path: Path,
) -> None:
    model_calls: list[dict] = []
    ...
    assert model_calls[0]["kwargs"]["word_timestamps"] is True
    assert [word.text for word in result.segments[0].words] == ["hello", "world"]
    assert result.segments[0].words[0].word_id == "word-000001"
    assert result.segments[0].words[0].start_seconds == 0.1
