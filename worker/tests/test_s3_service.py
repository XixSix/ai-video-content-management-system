...
from app.services.s3_service import S3Service, S3ServiceError, S3SourceObjectNotFoundError
...
class FakeClient:
    ...
    def download_file(self, bucket: str, object_key: str, destination_path: str) -> None:
        raise self.error
...
def test_download_file_keeps_transient_errors_retryable(tmp_path: Path) -> None:
    service = _service_with_error(EndpointConnectionError(endpoint_url="http://minio:9000"))
